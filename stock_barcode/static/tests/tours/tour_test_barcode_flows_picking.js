...
registry.category("web_tour.tours").add('test_delivery_reserved_lots_1', {test: true, steps: () => [
    ...
    { trigger: '.o_barcode_line.o_selected:not(.o_line_completed)', run: 'scan lot2' },
    {
        ...
        run: function() {
            ...
            helper.assertLineQty(sublines[0], "2 / 2");
            helper.assert(sublines[0].querySelector('button.o_add_quantity'), null);
            helper.assertLineQty(sublines[1], "1");
            helper.assert(sublines[1].querySelector('button.o_add_quantity'), null);
            helper.assertLineQty(sublines[2], "2 / 3");
            helper.assert(sublines[2].querySelector('button.o_add_quantity'), null);
        },
    },
    // Open the form view to trigger a save.
    { trigger: '.o_add_line' },
...
registry.category("web_tour.tours").add('test_pack_source_location', {test: true, steps: () => [
    {
        trigger: '.o_stock_barcode_main_menu:contains("Barcode Scanning")',
    },
    {
    ...
    },
    {
        ...
        run: function() {
            const line = helper.getLine({ barcode: 'product1' });
            helper.assertLineSourceLocation(line, ".../Section 4");
        },
    },
    ...stepUtils.validateBarcodeOperation(),
]});
...
registry.category("web_tour.tours").add("test_barcode_pack_lot_tour", { test: true, steps: () => [
    ...
    { trigger: '.o_barcode_line', run: "scan LOT002" },
    { trigger: '.o_line_button.o_toggle_sublines', run: 'click'},
    {
        extra_trigger: 'span.o_line_lot_name:contains("LOT002")',
        trigger: 'button.o_put_in_pack',
        run: 'click',
    },
    {
        extra_trigger: '.o_barcode_line:nth-child(2):has(".fa-archive")',
        trigger: '.o_barcode_line_summary',
        run: 'click',
    },
    { trigger: '.o_barcode_line_summary', run: "scan LOT002" },
    {
        extra_trigger: '.o_barcode_line.o_line_not_completed:contains(LOT002):not(:has(.fa-archive))',
        trigger: 'button.o_put_in_pack',
        run: 'click',
    },
    // Pack two units of the same reserved lot in different packages
    {
        extra_trigger: '.o_barcode_line:nth-child(3):has(".fa-archive")',
        trigger: '.o_barcode_line_summary',
        run: "scan LOT001" ,
    },
    {
        extra_trigger: '.o_barcode_scanner_qty:has(.d-inline-block:contains(3))',
        trigger: 'button.o_put_in_pack',
        run: 'click',
    },
    {
        extra_trigger: '.o_barcode_line:nth-child(4):has(".fa-archive")',
        trigger: '.o_barcode_line_summary',
        run: 'click',
    },
    { trigger: '.o_barcode_line_summary', run: "scan LOT001" },
    {
        extra_trigger: '.o_barcode_scanner_qty:has(.d-inline-block:contains(4))',
        trigger: 'button.o_put_in_pack',
        run: 'click',
    },
    {
        extra_trigger: '.o_barcode_line:nth-child(5):has(".fa-archive")',
        trigger: '.o_validate_page',
        run: 'click',
    },
    {
...
registry.category("web_tour.tours").add("test_validate_uncomplete_return", {
    ...
    steps: () => [
        ...
        { trigger: ".o_stock_barcode_main_menu" },
        { trigger: ".o_web_client:not(.modal-open)", run() {}},
    ],
});
